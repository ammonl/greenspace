...
| `monitoring.tf`  | CloudWatch log groups, SNS alarm topic, metric alarms, dashboard (alarms/dashboard gated), VPC flow logs (gated, retirable), module-wide `aws_caller_identity` / `aws_region` data sources |
...
Retirement is a one-way door — reverting `retire_dedicated_vpc` to
