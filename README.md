...
The roles grant least-privilege access to the S3 state backend, DynamoDB lock table, and the specific AWS resources managed by Terraform (the Lambda's security group in the shared VPC, IAM, KMS, CloudWatch Logs, Lambda, RDS, Secrets Manager).
